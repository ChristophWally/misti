        ...
        <style dangerouslySetInnerHTML={{
          __html: `
            ...
            .sketchy::after {
              ...
              bottom: 2px;
              background: 
                repeating-linear-gradient(23deg, transparent, transparent 1px, currentColor 1px, currentColor 1.5px, transparent 1.5px, transparent 4px),
                repeating-linear-gradient(67deg, transparent, transparent 1px, currentColor 1px, currentColor 1.5px, transparent 1.5px, transparent 6px);
              opacity: 0.05;
              border-radius: inherit;
              pointer-events: none;
            }
            ...
            .tag-essential.bg-blue-100 { background: #3b82f6 !important; color: white !important; }
            .tag-essential.bg-pink-100 { background: #ec4899 !important; color: white !important; }
            .tag-essential.bg-purple-100 { background: #8b5cf6 !important; color: white !important; }
            .tag-essential.bg-green-100, .tag-essential.bg-yellow-100 { background: #f59e0b !important; color: white !important; }
            .tag-essential.bg-red-100 { background: #ef4444 !important; color: white !important; }
            .tag-essential.bg-indigo-100 { background: #6366f1 !important; color: white !important; }
            .tag-essential.bg-gray-100 { background: #6b7280 !important; color: white !important; }
            
            .word-card.bg-teal-50 .tag-detailed.bg-teal-100 { background: #14b8a6 !important; color: white !important; }
            .word-card.bg-cyan-50 .tag-detailed.bg-cyan-100 { background: #0891b2 !important; color: white !important; }
            .word-card.bg-blue-50 .tag-detailed.bg-blue-100 { background: #3b82f6 !important; color: white !important; }
            .word-card.bg-purple-50 .tag-detailed.bg-purple-100 { background: #8b5cf6 !important; color: white !important; }
            ...
            .word-card.bg-teal-50 .audio-btn { background: #14b8a6 !important; color: white !important; }
            .word-card.bg-cyan-50 .audio-btn { background: #0891b2 !important; color: white !important; }
            .word-card.bg-blue-50 .audio-btn { background: #3b82f6 !important; color: white !important; }
            .word-card.bg-purple-50 .audio-btn { background: #8b5cf6 !important; color: white !important; }
            ...
            .search-input:focus { transform: rotate(-0.2deg); }
            .nav-btn { transition: all 0.2s ease; transform: rotate(0.3deg); }
            .nav-btn:hover { transform: rotate(-0.3deg) scale(1.02); }
            .nav-btn.primary { transform: rotate(-0.8deg); }
            .nav-btn.primary:hover { transform: rotate(0.8deg) scale(1.05); }
            .add-btn { transition: all 0.2s ease; transform: rotate(-0.5deg); }
            .add-btn:hover { transform: rotate(0.5deg) scale(1.05); }
            .dictionary-title { transform: rotate(-0.3deg); }
            .close-btn:hover { transform: rotate(15deg); }
            .word-italian { transform: rotate(-0.2deg); }
            .word-english { transform: rotate(0.1deg); }
            .filter-toggle { transform: rotate(0.2deg); transition: all 0.2s ease; }
            .filter-toggle:hover { transform: rotate(-0.2deg); }
            
            @media (max-width: 768px) {
              .tag-essential, .tag-detailed { font-size: 10px; padding: 3px 6px; }
              .audio-btn { width: 28px !important; height: 28px !important; }
              .audio-btn svg { width: 12px !important; height: 12px !important; }
            }
          `
        }} />
      </head>
          ...
          <div className="w-full px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              ...
              <div className="flex items-center space-x-4">
                <button id="dictionary-btn" className="nav-btn primary bg-emerald-600 text-white px-4 py-2 rounded-md hover:bg-emerald-700 transition-colors shadow-md">
                  📚 Dictionary
                </button>
                <button className="nav-btn text-white hover:text-cyan-200 transition-colors">My Decks</button>
                <button className="nav-btn text-white hover:text-cyan-200 transition-colors">Profile</button>
              </div>
            </div>
          </div>
        </nav>

        <div id="dictionary-panel" className="fixed inset-y-0 right-0 w-96 md:w-3/4 lg:w-2/3 xl:w-1/2 bg-white shadow-xl transform translate-x-full transition-transform duration-300 ease-in-out z-50 sketchy" style={{ minWidth: '384px', maxWidth: '80vw' }}>
          <div id="resize-handle" className="absolute left-0 top-0 w-1 h-full bg-teal-300 cursor-ew-resize hover:bg-teal-400 transition-colors opacity-0 hover:opacity-100"></div>
          <div className="flex flex-col h-full">
            <div className="flex items-center justify-between p-4 border-b bg-gradient-to-r from-teal-500 to-cyan-500">
              ...
              <button id="close-dictionary" className="close-btn text-white hover:text-cyan-200 transition-all">✕</button>
            </div>
            <div className="p-4 border-b bg-cyan-50">
              <div className="space-y-3">
                <input type="text" id="dictionary-search" placeholder="Search Italian words..." className="search-input w-full px-3 py-2 border border-teal-300 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 sketchy" />
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-white">
              <div id="dictionary-results">
                <div id="words-container" className="space-y-3"></div>
                <div id="loading" className="text-center py-4 text-teal-600">Loading words...</div>
                <div id="no-results" className="text-center py-4 text-gray-500 hidden">No words found</div>
              </div>
            </div>
          </div>
        </div>

        <div id="dictionary-overlay" className="fixed inset-0 bg-black bg-opacity-50 z-40 opacity-0 pointer-events-none transition-opacity duration-300"></div>

        <main className="min-h-screen">{children}</main>

        {/* COMPLETE JAVASCRIPT - This is the full system */}
...
class EnhancedDictionarySystem {
  ...
  async loadWords(searchTerm = '', filters = {}) {
    try {
      let query = this.supabase.from('dictionary').select('id,italian,english,word_type,tags,created_at,word_audio_metadata(id,audio_filename,azure_voice_name,duration_seconds)').order('italian', { ascending: true });
      if (searchTerm) query = query.or('italian.ilike.%' + searchTerm + '%,english.ilike.%' + searchTerm + '%');
      if (filters.wordType && filters.wordType.length > 0) query = query.in('word_type', filters.wordType);
      if (filters.tags && filters.tags.length > 0) filters.tags.forEach(tag => { query = query.contains('tags', [tag]); });
      if (filters.cefrLevel) query = query.contains('tags', ['CEFR-' + filters.cefrLevel]);
      const { data: words, error } = await query.limit(20);
      if (error) throw error;
      return await Promise.all(words.map(word => this.enhanceWordData(word)));
    } catch (error) {
  ...
  async enhanceWordData(word) {
    const enhanced = { ...word };
    if (word.word_type === 'NOUN') enhanced.articles = this.generateArticles(word);
  ...
  generateArticles(word) {
    ...
    return {
      ...
      indefinite: { singular: this.calculateIndefiniteArticle(italian, gender), plural: null }
    };
  }
  ...
  calculateArticle(word, gender, isPlural) {
    ...
    if (isPlural) {
      if (gender === 'masculine') {
        if (/[aeiou]/.test(firstChar) || ['gn','ps','sc','sp','st','x','z'].includes(firstTwo) || (firstChar === 's' && consonantAfterS)) return 'gli';
        return 'i';
      } else return 'le';
    } else {
      if (gender === 'masculine') {
        if (/[aeiou]/.test(firstChar) || ['gn','ps','sc','sp','st','x','z'].includes(firstTwo) || (firstChar === 's' && consonantAfterS)) return 'lo';
        return 'il';
      } else {
        if (/[aeiou]/.test(firstChar)) return "l'";
        return 'la';
      }
    }
  }
  ...
  calculateIndefiniteArticle(word, gender) {
    ...
    const consonantAfterS = word.length > 1 && /[bcdfghjklmnpqrstvwxyz]/.test(word.charAt(1));
    if (gender === 'masculine') {
      if (/[aeiou]/.test(firstChar) || ['gn','ps','sc','sp','st','x','z'].includes(firstTwo) || (firstChar === 's' && consonantAfterS)) return 'uno';
      return 'un';
    } else {
      if (/[aeiou]/.test(firstChar)) return "un'";
      return 'una';
    }
  }
    ...
    (tags || []).forEach(tag => {
      ...
      if (tagInfo) {
        if (tagInfo.essential) {
          essential.push({ tag, display: tagInfo.display, class: tagInfo.class, description: tagInfo.description });
        } else {
          detailed.push({ tag, display: tagInfo.display, class: tagInfo.class, description: tagInfo.description });
        }
      }
    });
    const wordTypeInfo = tagMap[wordType];
    if (wordTypeInfo) {
      detailed.unshift({ tag: wordType, display: wordTypeInfo.display, class: wordTypeInfo.class, description: wordTypeInfo.description });
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  let currentFilters = { wordType: [], cefrLevel: '', tags: [] };
  
  const grammarFiltersByType = {
    'NOUN': [
      { value: 'masculine', label: 'Masculine ♂' }, { value: 'feminine', label: 'Feminine ♀' }, { value: 'common-gender', label: 'Common ⚥' },
      { value: 'plural-i', label: 'Plural -i' }, { value: 'plural-e', label: 'Plural -e' }, { value: 'plural-invariable', label: 'Invariable' }
    ],
    'VERB': [
      { value: 'are-conjugation', label: '-are verbs' }, { value: 'ere-conjugation', label: '-ere verbs' }, { value: 'ire-conjugation', label: '-ire verbs' },
      { value: 'ire-isc-conjugation', label: '-isc verbs' }, { value: 'avere-auxiliary', label: 'Uses avere' }, { value: 'essere-auxiliary', label: 'Uses essere' },
      { value: 'transitive-verb', label: 'Transitive' }, { value: 'intransitive-verb', label: 'Intransitive' }
    ],
    'ADJECTIVE': [
      { value: 'form-4', label: '4 forms' }, { value: 'form-2', label: '2 forms' }, { value: 'form-invariable', label: 'Invariable' }, { value: 'type-gradable', label: 'Gradable' }
    ],
    'ADVERB': [
      { value: 'type-manner', label: 'Manner' }, { value: 'type-time', label: 'Time' }, { value: 'type-place', label: 'Place' }, { value: 'type-quantity', label: 'Quantity' }
    ],
    'ALL': [
      { value: 'irregular-pattern', label: 'Irregular ⚠️' }, { value: 'freq-top100', label: 'Top 100 ⭐' }, { value: 'freq-top500', label: 'Top 500 ⭐' },
      { value: 'native', label: 'Native 🗣️' }, { value: 'business', label: 'Business 💼' }
    ]
  };
  ...
  function updateGrammarFilters() {
    ...
    selectedTypes.forEach(type => {
      if (grammarFiltersByType[type]) applicableFilters = applicableFilters.concat(grammarFiltersByType[type]);
    });
    applicableFilters = applicableFilters.concat(grammarFiltersByType['ALL']);
    const uniqueFilters = applicableFilters.filter((filter, index, self) => index === self.findIndex(f => f.value === filter.value));
    grammarContainer.style.opacity = '0.5';
    grammarContainer.style.transform = 'translateY(-10px)';
    setTimeout(() => {
      grammarContainer.innerHTML = uniqueFilters.map(filter => '<span class="filter-chip ' + (currentFilters.tags.includes(filter.value) ? 'active' : '') + '" data-filter="tags" data-value="' + filter.value + '">' + filter.label + '</span>').join('');
  ...
  function createEnhancedWordElement(word) {
    ...
    const audioMetadata = word.word_audio_metadata && word.word_audio_metadata.length > 0 ? word.word_audio_metadata[0] : null;
    ...
    const essentialTags = word.processedTags.essential.map(tag => '<span class="tag-essential ' + tag.class + '" title="' + tag.description + '">' + tag.display + '</span>').join(' ');
    const detailedTags = word.processedTags.detailed.map(tag => '<span class="tag-detailed ' + tag.class + '" title="' + tag.description + '">' + tag.display + '</span>').join(' ');
  ...
  searchInput.addEventListener('input', function(e) {
    ...
    searchTimeout = setTimeout(() => { loadWords(e.target.value); }, 300);
  });
  ...
  async function playAudio(wordId, italianText, audioFilename) {
    ...
    try {
      const hasValidAudioFile = audioFilename && audioFilename !== 'null' && audioFilename !== null && audioFilename.trim() !== '' && typeof audioFilename === 'string';
      if (hasValidAudioFile) {
        let urlData, urlError;
        const result1 = await supabaseClient.storage.from('word-audio').createSignedUrl(audioFilename, 60);
        if (result1.data && result1.data.signedUrl) {
          urlData = result1.data;
          urlError = null;
        } else {
          const result2 = await supabaseClient.storage.from('audio-files').createSignedUrl(audioFilename, 60);
          urlData = result2.data;
          urlError = result2.error;
        }
        if (urlData && urlData.signedUrl) {
          const audio = new Audio(urlData.signedUrl);
          audio.onended = () => { audioBtn.innerHTML = originalHTML; audioBtn.disabled = false; };
          audio.onerror = (e) => { console.error('Error playing pregenerated audio:', e); fallbackToTTS(italianText, audioBtn, originalHTML); };
          await audio.play();
          return;
        }
      }
      ...
      console.error('General error in playAudio function:', error);
      fallbackToTTS(italianText, audioBtn, originalHTML);
    }
  }
  ...
  function fallbackToTTS(text, audioBtn, originalHTML) {
    if ('speechSynthesis' in window) {
      ...
      const setItalianVoice = (voices) => {
        const italianVoices = voices.filter(voice => voice.lang.startsWith('it') || voice.lang.includes('IT') || voice.name.toLowerCase().includes('ital'));
        if (italianVoices.length > 0) {
          const preferredVoice = italianVoices.find(voice => voice.name.includes('Luca') || voice.name.includes('Alice') || voice.name.includes('Federica') || voice.name.includes('Italia')) || italianVoices[0];
      ...
      const speakText = () => {
        utterance.onend = () => { audioBtn.innerHTML = originalHTML; audioBtn.disabled = false; };
        utterance.onerror = (event) => { console.error('Speech synthesis error:', event); audioBtn.innerHTML = originalHTML; audioBtn.disabled = false; };
        speechSynthesis.cancel();
        setTimeout(() => speechSynthesis.speak(utterance), 100);
      };
      let voices = speechSynthesis.getVoices();
      if (voices.length === 0) {
        speechSynthesis.onvoiceschanged = () => { voices = speechSynthesis.getVoices(); setItalianVoice(voices); speakText(); };
