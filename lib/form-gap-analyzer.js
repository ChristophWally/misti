...
export class FormGapAnalyzer {
  ...
  async runCompleteAnalysis(options = {}) {
    ...
    try {
      ...
      if (verbs.length === 0) {
        console.warn('⚠️ No verbs loaded for analysis - check priority filtering');
        // Try without priority filter as fallback
        const allVerbs = await this.loadVerbsForAnalysis(false, 10);
        console.log(`📊 Fallback: Found ${allVerbs.length} verbs without priority filter`);
        
        if (allVerbs.length > 0) {
          // Use first few verbs for analysis
          for (const verb of allVerbs.slice(0, 5)) {
      ...
      console.log('📊 Results summary:', {
        analyzedVerbs: this.analysisResults.analyzedVerbs,
        missingEpicForms: this.analysisResults.missingEpicForms.length,
        deprecatedForms: this.analysisResults.deprecatedForms.length,
        epicAlignmentIssues: this.analysisResults.epicAlignmentIssues.length
      });
  ...
  async analyzeVerbAgainstEpic(verb, checkDeprecated = true) {
    ...
    try {
      ...
      console.log(`📊 ${verb.italian}: Found ${forms?.length || 0} total forms`);
      ...
      if (conjugationForms.length > 0) {
        ...
        console.log(`🏷️ ${verb.italian}: All tags found:`, Array.from(allTags).sort());
      }

      // Check 1: Missing EPIC required forms
      const missingEpicForms = this.checkMissingEpicForms(verb, conjugationForms);
      console.log(`📊 ${verb.italian}: Missing EPIC forms: ${missingEpicForms.length}`);
      ...
      if (checkDeprecated) {
        ...
        console.log(`⚠️ ${verb.italian}: Deprecated forms: ${deprecatedForms.length}`);
        if (deprecatedForms.length > 0) {
          console.log(`⚠️ ${verb.italian}: Found deprecated:`, deprecatedForms);
      ...
      console.log(`🔧 ${verb.italian}: Incorrectly stored compounds: ${incorrectlyStoredCompounds.length}`);
      ...
      console.log(`🧱 ${verb.italian}: Missing building blocks: ${missingBuildingBlocks.length}`);
  ...
  async analyzeSpecificVerb(verbItalian) {
    ...
    try {
      ...
      // Reset results for single verb analysis
      this.analysisResults = {
        totalVerbs: 0,
        analyzedVerbs: 0,
        epicAlignmentIssues: [],
