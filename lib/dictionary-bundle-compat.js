...
export function hydrateBundleForm(form, context = {}) {
  ...
  // Fallback: if no form-level pronunciation links, try form_translation_group_link level.
  // Contracted preposition forms (del, dei, etc.) store audio on the FTG link record,
  // not the form record directly.
  const effectivePronunciationLinks = resolvedPronunciationLinks.length > 0
    ? resolvedPronunciationLinks
    : resolvedTranslationGroups.flatMap(rtg =>
        getEntityPronunciationLinks(
          'form_translation_group_link',
          rtg.form_translation_group_link_id,
          pronunciationLinks,
          pronunciationsById,
          mediaAssetsById
        )
  ...
  const primaryAudio = resolvePrimaryAudio(effectivePronunciationLinks)
  ...
  return {
    ...
    pronunciation_links: effectivePronunciationLinks,
    primary_audio: primaryAudio,
