...
export class EnhancedDictionarySystem {
  ...
  mapUiFiltersToRpcFilters(filters = {}) {
    ...
    const freqMap = {
      'freq-top100': 'top100',
      'freq-top500': 'top500',
      'freq-top1000': 'top1000',
      'freq-top5000': 'top5000',
    };
    ...
    for (const t of tagValues) {
      ...
      else if (freqMap[t]) add('metaattr007', freqMap[t]);
      else if (genderMap[t]) add('metaattr011', genderMap[t]);
      else if (pluralMap[t]) add('metaattr026', pluralMap[t]);
  ...
  /**
   * Transform a listing row to the legacy word object WordCard expects.
   * - id, italian, word_type
   * - word_translations: ordered by display_priority, with rpc tags attached
   * - tags: derive from word_core_tags for header chips (best-effort mapping)
   */
  transformListingRowToWord(row) {
    const coreTags = Array.isArray(row.word_core_tags) ? row.word_core_tags : [];
    const optTags = Array.isArray(row.word_optional_tags) ? row.word_optional_tags : [];

    // Best-effort mapping from normalized to legacy tag strings used by WordCard
    const tags = [];
    let auxSet = new Set();
    let auxShorts = new Set();
    for (const t of coreTags) {
      switch (t.attribute_stable_id) {
        case 'metaattr003': // CEFR Level
          if (t.value_label) tags.push(`CEFR-${t.value_label}`);
          break;
        case 'metaattr007': // Frequency Tier
          if (t.value_label) tags.push(`freq-${t.value_label}`);
          break;
        case 'metaattr002': // Auxiliary Verb propagated to word (COMBINE)
          if (t.value_label) auxSet.add(String(t.value_label).toLowerCase());
          if (t.value_shorthand) auxShorts.add(String(t.value_shorthand));
          else if (t.value_label) auxShorts.add(String(t.value_label).toUpperCase());
          break;
        case 'metaattr011': // Gender (word-level noun gender)
          if (t.value_label === 'masculine' || t.value_label === 'feminine') tags.push(t.value_label);
          if (t.value_label === 'common') tags.push('common-gender');
          break;
        case 'metaattr004': // Conjugation Type
          if (String(row.word_type).toUpperCase() === 'VERB' && t.value_label) {
            tags.push(`${t.value_label}-conjugation`);
          }
          break;
        case 'metaattr026': // Plural Formation
          if (t.value_label) tags.push(t.value_label);
          break;
        case 'metaattr017': // Reflexive
          if (t.value_label === 'reflexive' && row.word_type === 'VERB') tags.push('reflexive-verb');
          break;
        case 'metaattr006': // Form Pattern (adjective forms)
          if (t.value_label) tags.push(t.value_label);
          break;
        case 'metaattr009': // Gradable mapping
          if (t.value_label) {
            const gv = String(t.value_label).toLowerCase();
            if (gv === 'both') tags.push('full-grade');
            else if (gv === 'analytical') tags.push('analytical-grade');
            else if (gv === 'false') tags.push('non-grade');
          }
          break;
        default:
          break;
    ...
    // Build word-level auxiliary summary (combined shorthands)
    const aux_summary = Array.from(auxShorts).join('/');
    ...
    const word_translations = translations.map((tr) => ({
      ...
      // Attach normalized tags for restriction/icons usage in UI
