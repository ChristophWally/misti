...
export class EnhancedDictionarySystem {
  ...
  async loadWordsNormalized(searchTerm = '', filters = {}) {
    try {
      // Use the same proven selection pattern as the pre-change implementation
      let query = this.supabase
        .from('dictionary')
        .select(`
          *,
        ...
        `)
        .order('italian', { ascending: true })
        // Ensure child rows (translations) come back ordered by display_priority
        .order('display_priority', { foreignTable: 'word_translations', ascending: true })
        .limit(20);

      if (searchTerm) {
        // Use ilike on italian field for predictable matching
        query = query.ilike('italian', `%${searchTerm}%`);
      }
      if (filters.wordType && filters.wordType.length > 0) {
        query = query.in('word_type', filters.wordType);
      }

      // Apply overlaps filters to match previous behavior
      if (filters.tags && filters.tags.length > 0) {
        query = query.overlaps('tags', filters.tags);
      }
      if (filters.cefrLevel) {
      ...
      const translationIds = (words || []).flatMap((w) =>
        (w.word_translations || []).map((t) => t.id)
      );

      // Early return if nothing to fetch
      if (!translationIds.length) return words || [];

      const results = new Map();

      // Chunk calls to avoid large payloads
      ...
      try {
        for (const batch of chunk(translationIds)) {
          const { data: rpcRows, error: rpcError } = await this.supabase.rpc(
            'app_get_translation_tags',
            { p_translation_ids: batch }
          );
          if (rpcError) throw rpcError;
          (rpcRows || []).forEach((row) => results.set(row.translation_id, row));
        }
      } catch (rpcErr) {
        console.warn('RPC app_get_translation_tags failed:', rpcErr?.message || rpcErr);
        // Proceed without optional tags; base words still render with core chips
      }

      // Attach tags to translations (as rpc_tags)
      return (words || []).map((w) => ({
        ...w,
        word_translations: (w.word_translations || []).map((t) => ({
          ...t,
          rpc_tags: (results.get(t.id)?.tags) || [],
          rpc_core: (results.get(t.id)?.core) || null,
        })),
      }));
    } catch (e) {
      // Surface error to caller; legacy fallback removed to avoid duplicate code paths
      throw e;
    }
  }
