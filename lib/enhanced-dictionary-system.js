...
import {
  deriveLegacyWordTags,
  deriveTranslationContextMetadata,
  getPrimaryAudioDescriptor,
  hydrateCanonicalWordBundle,
} from './dictionary-bundle-compat'
...
export class EnhancedDictionarySystem {
  ...
  // Legacy method removed - application now uses loadWordsNormalized() which uses RPC system
  ...
  mapUiFiltersToRpcFilters(filters = {}) {
    ...
    // Number Filter -> metaattr012 values: singolare/plurale
    if (filters.numberFilter && typeof filters.numberFilter === 'string' && filters.numberFilter.trim()) {
      out.push({ attribute: 'metaattr012', values: [filters.numberFilter.trim()] });
    }

    // Number Restriction Filter -> metaattr013 values: singular-only/plural-only
    if (filters.numberRestriction && typeof filters.numberRestriction === 'string' && filters.numberRestriction.trim()) {
      out.push({ attribute: 'metaattr013', values: [filters.numberRestriction.trim()] });
    }
    ...
    // Plural Formation (metaattr026): plural-e/plural-i
    const pluralMap = {
      'plural-e': 'plural-e',
      'plural-i': 'plural-i',
      // Note: 'invariable' and 'irregular' values removed - they don't exist in meta_values system
    };
  ...
  transformListingRowToWord(row) {
    ...
    const wordDirectCoreTags = row.word_direct_core_tags || [];
    const wordInheritedCoreTags = row.word_inherited_core_tags || [];
    const wordDisplayCoreTags = row.word_display_core_tags || row.word_core_tags || [];
    const wordOptionalTags = row.word_optional_tags || [];
    const pronunciationGroups = Array.isArray(row.pronunciation_groups)
      ? row.pronunciation_groups
      : [];
    const primaryPronunciationGroup = row.primary_pronunciation_group || pronunciationGroups[0] || null;
    const primaryAudio = primaryPronunciationGroup?.primary_audio
      ? {
          bucket: primaryPronunciationGroup.primary_audio.storage_bucket || null,
          object_key: primaryPronunciationGroup.primary_audio.object_key || null,
          voice_name: primaryPronunciationGroup.primary_audio.voice_name || null,
          ipa_pronunciation: primaryPronunciationGroup.ipa_pronunciation || null,
          phonetic_pronunciation: primaryPronunciationGroup.phonetic_pronunciation || null,
        }
      : row.primary_media
      ? {
          bucket: row.primary_media.storage_bucket || null,
          object_key: row.primary_media.object_key || null,
          voice_name: row.primary_media.voice_name || null,
          ipa_pronunciation: row.primary_pronunciation?.ipa_pronunciation || null,
          phonetic_pronunciation: row.primary_pronunciation?.phonetic_pronunciation || null,
        }
      : null
    const word_translations = translations.map((tr) => ({
      ...
      metadata: deriveTranslationContextMetadata(tr.core_tags || [], tr.context_metadata || null),
      context_metadata: deriveTranslationContextMetadata(tr.core_tags || [], tr.context_metadata || null),
    ...
    return {
      ...
      audio_bucket: primaryAudio?.bucket || null,
      audio_object_key: primaryAudio?.object_key || null,
      audio_voice_name: primaryAudio?.voice_name || null,
      primary_ipa: primaryAudio?.ipa_pronunciation || row.primary_pronunciation?.ipa_pronunciation || null,
      primary_phonetic:
        primaryAudio?.phonetic_pronunciation || row.primary_pronunciation?.phonetic_pronunciation || null,
      
      // Pass RPC word core tags through directly
      word_core_tags: wordDisplayCoreTags,
      word_direct_core_tags: wordDirectCoreTags,
      word_inherited_core_tags: wordInheritedCoreTags,
      word_display_core_tags: wordDisplayCoreTags,
      word_optional_tags: wordOptionalTags,
      
      // Legacy fields for backward compatibility (if needed)
      tags: deriveLegacyWordTags(wordDisplayCoreTags, wordOptionalTags),
  ...
  async getConjugationPreview(wordId) {
    try {
      const { data, error } = await this.supabase.rpc('app_get_word_bundle', {
        p_word_id: wordId,
      });

      if (error) throw error;

      const bundlePayload = Array.isArray(data) ? data[0] : data;
      const bundle = hydrateCanonicalWordBundle(bundlePayload || {});
      const forms = Array.isArray(bundle?.forms) ? bundle.forms : [];

      return forms
        .filter((form) => {
          const labels = [
            ...(form.core_tags || []).map((tag) => tag?.value_label),
            ...(form.optional_tags || []).map((tag) => tag?.value_label),
          ].filter(Boolean);

          return (
            form.form_type === 'conjugation' &&
            labels.includes('indicativo') &&
            labels.includes('presente')
          );
        })
        .slice(0, 3)
        .map((form) => ({
          form_text: form.form_text,
          translation: form.resolved_translation_groups?.[0]?.translation || null,
          tags: [
            ...(form.core_tags || []).map((tag) => tag?.value_label),
            ...(form.optional_tags || []).map((tag) => tag?.value_label),
          ].filter(Boolean),
  ...
  checkPremiumAudio(word) {
    ...
    const filename = primaryAudio?.object_key || null;
    const hasPremiumAudio = !!filename;
    
    return { 
      hasPremiumAudio, 
      audioFilename: hasPremiumAudio ? filename : null,
      audioBucket: primaryAudio?.bucket || null,
      audioObjectKey: hasPremiumAudio ? filename : null,
