...
export class EnhancedDictionarySystem {
  ...
  transformListingRowToWord(row) {
    ...
    const wordCoreTags = row.word_core_tags || [];
    const wordOptionalTags = row.word_optional_tags || [];
    const primaryAudio = row.primary_media
      ? {
    ...
    return {
      ...
      primary_ipa: row.primary_pronunciation?.ipa_pronunciation || null,
      primary_phonetic: row.primary_pronunciation?.phonetic_pronunciation || null,
      
      // Pass RPC word core tags through directly
      word_core_tags: wordCoreTags,
      word_optional_tags: wordOptionalTags,
      
      // Legacy fields for backward compatibility (if needed)
      tags: deriveLegacyWordTags(wordCoreTags, wordOptionalTags),
