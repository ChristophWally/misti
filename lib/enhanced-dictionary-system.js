...
export class EnhancedDictionarySystem {
  ...
  transformListingRowToWord(row) {
    ...
    for (const t of coreTags) {
      switch (t.attribute_stable_id) {
        ...
        case 'metaattr006': // Form Pattern (adjective forms)
          if (t.value_label) tags.push(t.value_label);
          break;
        case 'metaattr009': // Gradable
          if (t.value_label === 'both') tags.push('type-gradable');
          if (t.value_label === 'absolute') tags.push('type-absolute');
          break;
        default:
          break;
      }
    ...
    // Add word-level auxiliary summary chip: avere, essere, or both
    if (auxSet.size === 1) {
      const v = Array.from(auxSet)[0];
      if (v === 'avere') tags.push('avere-auxiliary');
      if (v === 'essere') tags.push('essere-auxiliary');
    } else if (auxSet.size > 1) {
      tags.push('both-auxiliary');
    }
