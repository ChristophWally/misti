...
import { deriveLegacyWordTags, deriveTranslationContextMetadata } from './dictionary-bundle-compat'
...
export class EnhancedDictionarySystem {
    ...
    try {
      ...
      return forms
        ...
        .map((form) => ({
          ...
          translation: form.form_translations?.[0]?.translation || null,
          tags: [
            ...(form.core_tags || []).map((tag) => tag?.value_label),
            ...(form.optional_tags || []).map((tag) => tag?.value_label),
          ].filter(Boolean),
  ...
  checkPremiumAudio(word) {
    const audioMetadata = word.word_audio_metadata && word.word_audio_metadata.length > 0 
      ...
      : null;
    const fallbackFilename = typeof word.audio_filename === 'string' && word.audio_filename.trim() !== ''
      ? word.audio_filename
      : null;
    const filename = audioMetadata && audioMetadata.audio_filename && audioMetadata.audio_filename.trim() !== ''
