...
- [ ] **Migration Recommendation Engine** (`lib/migration/migrationRecommendationEngine.ts`) - Unified migration system that analyzes current data state and recommends which existing migration rules to execute, including terminology conversion through the existing rule engine
- [ ] **Migration execution interface** - Backend integration for safe transformations using the unified recommendation engine
