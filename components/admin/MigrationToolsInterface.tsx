...
export default function MigrationToolsInterface() {
  ...
  const runTagAnalysis = async () => {
    ...
    try {
      ...
      const { data: mixedTerminologyForms } = await supabase
        .from('word_forms')
      ...
      const { data: universalTerminologyForms } = await supabase
        .from('word_forms')
        .select('id, form_text, tags')
        .contains('tags', ['prima-persona']);

      const mixedCount = (mixedTerminologyForms?.length || 0);
      const universalCount = (universalTerminologyForms?.length || 0);

      addToDebugLog(`📊 Found ${mixedCount} forms with legacy terminology, ${universalCount} with universal terminology`);
      ...
      if (mixedCount > 0) {
        issues.push({
          ...
          description: `${mixedCount} forms using legacy Italian person terms (io, tu, lui, etc.)`,
          affectedCount: mixedCount,
            ...
            {debugLog.length > 0 && (
              <div className="bg-gray-900 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-sm font-medium text-white">Analysis Log</h4>
                  <button
                    onClick={() => setDebugLog([])}
